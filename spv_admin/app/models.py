...
class TokenizationSpec(models.Model):
    name = models.CharField(max_length=100, unique=True)   # e.g. “80-20-8pct”
    senior_pct = models.DecimalField(max_digits=5, decimal_places=2)   # 0-100
    junior_pct = models.DecimalField(max_digits=5, decimal_places=2)
    senior_coupon_pct = models.DecimalField(max_digits=5, decimal_places=2)  # 8.00
    senior_cap_method = models.CharField(                # simple, irr, custom
        max_length=20, choices=[("simple", "Simple"), ("irr", "IRR")],
        default="simple"
    )

    def clean(self):
        if not (0 < self.senior_pct < 100 and 0 < self.junior_pct < 100):
            raise ValidationError("Both tranche percentages must be between 0 and 100.")
        
        if self.senior_pct + self.junior_pct > 100:
            raise ValidationError("Senior + Junior percentages may not exceed 100.")
...
class Loan(models.Model):
    ...
    senior_id = models.BigIntegerField(null=True, blank=True)
    junior_id = models.BigIntegerField(null=True, blank=True)
    ...
    tranches = models.BooleanField(default=False)  # Whether the loan has tranches
    tokenization_spec = models.ForeignKey(
        TokenizationSpec, on_delete=models.PROTECT, null=True, blank=True
    )
    ...
    def monthly_interest(self) -> Decimal:
        return (self.principal * (self.annual_interest_rate / Decimal(100))) / Decimal(12)
